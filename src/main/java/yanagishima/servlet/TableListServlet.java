...
public class TableListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request,
        ...
        try {
            ...
            String query = String.format("SELECT table_catalog || '.' || table_schema || '.' || table_name FROM %s.information_schema.tables WHERE table_schema NOT IN %s", catalog, notin);
            List<String> tables = prestoService.doQuery(datasource, query, userName, false).getRecords().stream().map(list -> list.get(0)).collect(Collectors.toList());
